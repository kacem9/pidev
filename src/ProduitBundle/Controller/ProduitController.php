...
class ProduitController extends Controller
{
     ...
     */
    /*
public function ajaxrateAction($id){
    $em=$this->getDoctrine()->getManager();
    $rateavg=0;

        $cmt = $em->getRepository(CommentaireProd::class)->findByavg(array('produit'=>$id));
        if(isset($cmt)){
            $rateavg=$cmt;

        }
     return new Response(json_encode($rateavg));
    }*/
...
{  $user = $this->container->get('security.token_storage')->getToken()->getUser();
    $em = $this->getDoctrine()->getManager();
    $produits = $em->getRepository('ProduitBundle:Produit')->getBripref($user->getId());




    return $this->render('produit/fav.html.twig', array(
        'prd' => $produits,
    ));
}
